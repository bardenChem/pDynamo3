...
class MMModel ( EnergyModel ):
    ...
    def CompleteConnectivity ( self, target ):
        ...
        if connectivity is not None:
            ...
            # DEBUG: Print bond information
            print(f"DEBUG: Collected {len(bonds)} bonds from {len(state.mmTerms)} mmTerms")
            
            # Remove duplicate bonds before adding to connectivity
            if bonds:
                unique_bonds = []
                bond_set = set()
                duplicate_count = 0
                ...
                for bond in bonds:
                    # Handle different bond formats: (i, j) or (i, j, parameters)
                    if hasattr(bond, '__iter__') and len(bond) >= 2:
                        atom1, atom2 = bond[0], bond[1]
                        # Create unique identifier (sorted atom indices)
                        ...
                        if bond_key not in bond_set:
                            ...
                            duplicate_count += 1
                            print(f"DEBUG: Duplicate bond found: {atom1}-{atom2}")
                    else:
                        # Keep bonds that don't match expected format
                        unique_bonds.append(bond)
                
                if duplicate_count > 0:
                    print(f"DEBUG: Removed {duplicate_count} duplicate bonds")
                    bonds = unique_bonds
