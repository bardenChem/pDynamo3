...
class PDBFileReader ( TextFileReader ):
    ...
    def __ParseAtomRecord ( self, line, isHeteroatom ):
        ...
        # . Initialize atom counter for this component if it doesn't exist
        if not hasattr(self, '_component_atom_counter'):
            self._component_atom_counter = {}
    
        component_key = f"{entityLabel}:{componentLabel}"
    
        if component_key not in self._component_atom_counter:
            self._component_atom_counter[component_key] = {}
    
        # . If atom already exists with this name, rename it
        original_name = name
        if atom is not None:
            # . This atom name already exists in this component
            if name not in self._component_atom_counter[component_key]:
                self._component_atom_counter[component_key][name] = 1
            else:
                self._component_atom_counter[component_key][name] += 1
        
            # . Create new unique name (preserve first character if it's a letter)
            counter = self._component_atom_counter[component_key][name]
        
            # . For CO2 residues, we want names like O1, O2 instead of just O
            # . PDB atom names are 4 characters max
            if len(name) < 4:
                # . Simple case: append number
                new_name = f"{name}{counter}"
                new_name = new_name[:4]  # Truncate to 4 chars if needed
            else:
                # . Complex case: replace last character(s)
                base_name = name.rstrip('0123456789')
                if base_name != name:
                    # . Already has a number, increment it
                    try:
                        num_part = name[len(base_name):]
                        new_num = int(num_part) + counter
                        new_name = f"{base_name}{new_num}"
                    except:
                        new_name = f"{name[:3]}{counter}"
                else:
                    new_name = f"{name[:3]}{counter}"
        
            # . Update name and search again
            name = new_name
            ( entity, component, atom ) = self.currentModel.GetDescendantsFromLabels ( entityLabel, componentLabel, name )
    
        # . Store the counter for this atom name
        if original_name not in self._component_atom_counter[component_key]:
            self._component_atom_counter[component_key][original_name] = 1
        else:
            self._component_atom_counter[component_key][original_name] += 1
