...
class GromacsDefinitionsFileReader ( GromacsFileReader ):
    ...
    def ToSequence ( self ):
        """Generate a sequence without modifying atom names that affect parameters."""
        ...
        for i, datum in enumerate(self.atoms):
            chain = datum[8] if len(datum) > 8 else "A"
            res_name = datum[3] if len(datum) > 3 else "UNK"
            res_id = str(datum[2]) if len(datum) > 2 else "1"
            atom_name = datum[4] if len(datum) > 4 else f"AT{i+1}"
            
            # Create base path WITHOUT modifying the actual atom name
            base_path = f"{chain}{majorSeparator}{res_name}{minorSeparator}{res_id}{majorSeparator}{atom_name}"
            
            # Handle duplicates by adding a unique identifier to the PATH, not the atom name
            if base_path in path_count:
                path_count[base_path] += 1
                # Add counter to residue ID in the PATH only, not the actual data
                unique_path = f"{chain}{majorSeparator}{res_name}{minorSeparator}{res_id}_{path_count[base_path]}{majorSeparator}{atom_name}"
            else:
                path_count[base_path] = 1
                unique_path = base_path
                
            atomPaths.append(unique_path)
        
        # Log duplicates for information
        duplicates = {path: count for path, count in path_count.items() if count > 1}
        #if duplicates:
        #    print(f"Info: Made {len(duplicates)} duplicate paths unique for sequence generation")
