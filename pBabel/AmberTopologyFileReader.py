...
class AmberTopologyFileReader ( TextFileReader ):
    ...
    def ToSequence ( self ):
        """Assign a sequence to a system with duplicate handling."""
        # . Check for residues.
        ...
        if nresidues > 0:
            ...
            # . Initialization.
            atomPaths = []
            path_count = {}  # Track duplicate paths
            ...
            for ires in range ( nresidues ):
                ...
                for iatom in range ( start, stop ):
                    base_path = pathHead + atomnames[iatom]
                    
                    # Handle duplicates
                    if base_path in path_count:
                        path_count[base_path] += 1
                        unique_path = f"{base_path}_dup{path_count[base_path]}"
                    else:
                        path_count[base_path] = 1
            ...
            #if duplicates:
                #print(f"Info: Made {len(duplicates)} duplicate paths unique in AMBER topology")
